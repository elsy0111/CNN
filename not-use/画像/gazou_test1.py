...
acti_out = layers.Activation(
    'softmax'                       # 活性化関数（出力層用）： tanh関数（固定）
    , name='acti_out'               # 活性化関数にも名前付け
    )

#---------------------------------------------------
# 定数（モデル定義時に必要となる数値）適当です
INPUT_FEATURES = 1024  # 入力（特徴）の数： 32*32=1024
LAYER1_NEURONS = 3     # ニューロンの数： 3
LAYER2_NEURONS = 3     # ニューロンの数： 3
OUTPUT_RESULTS = 3    # 出力結果の数： 10
...
"""FileNotFoundError (model.png)"""
# モデルの構成図を表示
tf.keras.utils.plot_model(f_model, show_shapes=True, show_layer_names=True, to_file='画像\model.png')
Image(retina=False, filename='画像\model.png')

#---------------------------------------------------
"""# ValueErrorの発生、修正が必要(compileかfitがダメっぽい、わからん！)"""
model.compile(tf.keras.optimizers.SGD(learning_rate=0.03), 'mean_squared_error', [tanh_accuracy])
hist = model.fit(x_train, y_train, batch_size= 4, epochs=5, verbose=1, validation_data=(x_test, y_test))
"""
# これもダメだった
model.compile(
    optimizer="adam",
